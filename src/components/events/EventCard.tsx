...
const EventCard: React.FC<EventCardProps> = ({ event }) => {
  const { toggleLike, likedEvents } = useEvents();
  ...
  return (
    <Card 
      className="h-full cursor-pointer transition-transform duration-300 hover:-translate-y-1 hover:shadow-lg"
    ...
    >
      <div className="relative h-48 overflow-hidden">
        ...
        />
        <div className="absolute top-0 right-0 bg-violet-600 text-white px-3 py-1 m-2 rounded-full text-sm font-medium">
          {event.category}
        </div>
      </div>
      
      <CardContent className="pt-4">
        <h3 className="text-xl font-semibold line-clamp-2 mb-2 text-gray-900">{event.title}</h3>
        <p className="text-gray-600 line-clamp-3 mb-4">{event.shortDescription}</p>
      ...
      <CardFooter className="flex justify-between border-t border-gray-100">
        <button 
          className={`flex items-center ${isLiked ? 'text-red-500' : 'text-gray-500'} hover:text-red-600 transition-colors`}
